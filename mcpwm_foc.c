...
static void run_pid_control_speed(float dt, volatile motor_all_state_t *motor) {
	...
	float p_term;
	float d_term;
	static float output_buffer_spid = 0;
	static float max_output_change = 10; //max normalised change of output current per second
	static float output_change;
	
	output_change = max_output_change * dt;
	...
	if (motor->m_control_mode != CONTROL_MODE_SPEED) {
		...
		output_buffer_spid = motor->m_iq_set / conf_now->lo_current_max;	//store last current to buffer so transition from current control to pid control is smooth
		return;
	}
	...
	// Ensure some ramping on output change
	if(output > output_buffer_spid+output_change)
	{
		output = output_buffer_spid+output_change;
	}
	else if(output < output_buffer_spid-output_change)
	{
		output = output_buffer_spid-output_change;
	}
	
	output_buffer_spid = output;
	
	// Truncate it again just to be sure
	utils_truncate_number(&output, -1.0, 1.0);
