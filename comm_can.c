...
static void decode_msg(uint32_t eid, uint8_t *data8, int len, bool is_replaced) {
	...
	if (id == 255 || id == id1 || id == id2) {
		...
		case CAN_PACKET_PROCESS_SHORT_BUFFER:
			...
			//custom code to control pid parameters in runtime	
			case CAN_PACKET_CONF_PID: {
				ind = 0;
				
				float kp = buffer_get_float16(data8, 1e5, &ind);
				float ki = buffer_get_float16(data8, 1e5, &ind);
				float kd = buffer_get_float16(data8, 1e5, &ind);
				float ramp = buffer_get_float16(data8, 0.01, &ind);
				...
				*mcconf = *mc_interface_get_configuration();
				...
				mcconf->s_pid_ki = ki;
				mcconf->s_pid_kd = kd;
				mcconf->s_pid_ramp_erpms_s = ramp;
			...
			case CAN_PACKET_CONF_CURRENTS: {
				ind = 0;
				float min = buffer_get_float16(data8, 1e1, &ind);
				float max = buffer_get_float16(data8, 1e1, &ind);
				float min_in = buffer_get_float16(data8, 1e1, &ind);
				float max_in = buffer_get_float16(data8, 1e1, &ind);
				

				mc_configuration *mcconf = mempools_alloc_mcconf();
				*mcconf = *mc_interface_get_configuration();

				if (mcconf->l_current_min != min || mcconf->l_current_max != max || mcconf->l_in_current_min != min_in || mcconf->l_in_current_max != max_in) 
				{
					mcconf->l_current_min = min;
					mcconf->l_current_max = max;
					mcconf->l_in_current_min = min_in;
					mcconf->l_in_current_max = max_in;
